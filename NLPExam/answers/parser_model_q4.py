...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.exam_layer_w = nn.Parameter(torch.empty(self.hidden_size, self.hidden_size))
        nn.init.xavier_uniform_(self.exam_layer_w)
        self.exam_layer_b = nn.Parameter(torch.empty(self.hidden_size))
        nn.init.uniform_(self.exam_layer_b)
    ...
    def forward(self, w):
        ...
        h = F.relu(torch.add(torch.matmul(embedding, self.embed_to_hidden_weight), self.embed_to_hidden_bias))
        dropout_output = self.dropout(h)
        h2 = F.relu(torch.add(torch.matmul(dropout_output, self.exam_layer_w), self.exam_layer_b))
        logits = torch.add(torch.matmul(h2, self.hidden_to_logits_weight), self.hidden_to_logits_bias)
