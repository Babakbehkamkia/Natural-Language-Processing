...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncate_size = random.randint(4, int(self.block_size*7/8)+1)
        document = document[:truncate_size]
        
        masked_content_size = int(random.triangular(0, truncate_size, truncate_size/4))
        prefix_size = (truncate_size - masked_content_size)//2
        prefix = document[:prefix_size]
        masked_content = document[prefix_size:prefix_size+masked_content_size]
        suffix = document[prefix_size+masked_content_size:]
