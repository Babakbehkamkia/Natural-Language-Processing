...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        len_truncate = random.randint(4, int(self.block_size*7/8))
        document = document[:len_truncate]
        
        len_doc = int(len_truncate/4)
        len_masked_content = len_doc + random.randint(-int(len_doc/2), int(len_doc/2))
        len_prefix = int((len_truncate - len_masked_content)/2)
        prefix = document[:len_prefix]
        masked_content = document[len_prefix:len_prefix+len_masked_content]
        suffix = document[len_prefix+len_masked_content:]
