...
class DownProjectBlock(nn.Module):
    ...
    def forward(self, x_input):
        ...
        attention_output = self.attn(x_input, self.ln1(self.C))
        output = attention_output + self.mlp(self.ln2(attention_output))
        return output
        ### END YOUR CODE
...
class UpProjectBlock(nn.Module):
    ...
    def forward(self, y, x_input):
        ...
        ### Hint: Copy over the code from Block and make necessary modifications.
        ### Should be around 3-5 lines.
        attention_output = self.attn(self.ln1(y), x_input)
        output = y + self.mlp(self.ln2(x_input + attention_output))
        return output
        ### END YOUR CODE
