...
from torch.nn import LSTM, LSTMCell, Linear, Conv1d, Dropout
...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = Conv1d(in_channels=embed_size,
                                              out_channels=embed_size,
                                              kernel_size=2,
                                              padding='same')

        self.encoder = LSTM(input_size=embed_size,
                                     hidden_size=self.hidden_size,
                                     bias=True,
                                     bidirectional=True)

        self.decoder = LSTMCell(input_size=embed_size + self.hidden_size,
                                         hidden_size=self.hidden_size,
                                         bias=True)

        self.h_projection = Linear(self.hidden_size * 2, self.hidden_size, bias=False)
        self.c_projection = Linear(self.hidden_size * 2, self.hidden_size, bias=False)
        self.att_projection = Linear(self.hidden_size * 2, self.hidden_size, bias=False)
        self.combined_output_projection = Linear(self.hidden_size * 2, 1, bias=False)
        self.target_vocab_projection = Linear(self.hidden_size, len(self.vocab.tgt))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        print("size of dec_hidden: ", dec_hidden.size())
        U_t = torch.bmm(dec_hidden.unsqueeze(-1), a_t.unsqueeze(-1).permute(0,2,1))
        print("size of U_t: ", U_t)

        V_t = self.combined_output_projection(U_t).squeeze(-1)
