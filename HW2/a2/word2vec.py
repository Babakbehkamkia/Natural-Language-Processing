...
def negSamplingLossAndGradient(
    ...
    outside_loss = -np.log(sigmoid(np.matmul(outsideVectors[outsideWordIdx], centerWordVec)))
    negetive_loss = -np.sum(np.log(sigmoid(-np.matmul(outsideVectors[negSampleWordIndices], centerWordVec))))
    loss = outside_loss + negetive_loss


    outsid_gradCenter = -np.multiply(outsideVectors[outsideWordIdx], 1 - sigmoid(np.matmul(outsideVectors[outsideWordIdx].T, centerWordVec)))
    single_neg_grad = 1 - sigmoid(-np.matmul(outsideVectors[negSampleWordIndices], centerWordVec))[:, np.newaxis]
    neg_gradCenter = np.sum(np.multiply(outsideVectors[negSampleWordIndices], single_neg_grad), axis=0)
    gradCenterVec = outsid_gradCenter + neg_gradCenter

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    neg_grads = np.multiply(single_neg_grad, centerWordVec.T)

    for i in range(K):
        gradOutsideVecs[negSampleWordIndices[i]] += neg_grads[i]
    # gradOutsideVecs[negSampleWordIndices] += neg_grads
    gradOutsideVecs[outsideWordIdx] = -np.multiply((1 - sigmoid(np.matmul(outsideVectors[outsideWordIdx], centerWordVec))), centerWordVec.T)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i in range(2*windowSize):
        ...
        #     continue
        try:
            temp_loss, temp_gradCenterVec, temp_gradOutsideVectors = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[outsideWords[i]], outsideVectors, dataset)
            loss += temp_loss
